...
### 6. Frontend Deployment — Vercel (production) & Firebase Hosting (optional)

**The production frontend is deployed on Vercel** (`https://sovikjewels.vercel.app`). The SPA rewrite in `vercel.json` (`"/(.*)" → "/index.html"`, applied **after** the filesystem check) is what makes clean URLs work on **direct navigation and refresh** — without it, deep links such as `/admin`, `/shop` or `/product/:slug` return Vercel's `404: NOT_FOUND` because no static file exists at those paths.

Pushing to `main` triggers the Vercel Git integration automatically, or deploy manually with:

```bash
npm run deploy:vercel
```

Static assets (`/assets/*.js|css`, `/favicon.svg`) are real files, so Vercel's filesystem always serves them directly — the rewrite never swallows them. Hashed asset filenames get immutable caching via the `headers` config in `vercel.json`, while `index.html` is always revalidated so refreshes pick up new deploys immediately.
...
**Firebase Hosting remains available as an optional mirror** (`firebase.json` carries the equivalent `** → /index.html` rewrite). Firebase itself still powers the backend services: Firestore, Authentication, and the `generateProductDescription` Cloud Function (called directly at `us-central1-sovik-jewels.cloudfunctions.net`, independent of the frontend host). Deploy that mirror with:
